...
{
    double radians = degress_to_radians(angle);
    sin_theta = sin(radians);
    cos_theta = cos(radians);

    hasbox = ptr->bounding_box(0, 1, bbox);

    point3 min(inifinity, inifinity, inifinity);
    point3 max(-inifinity, -inifinity, -inifinity);
...
bool rotate_y::hit(const ray& r, double t_min, double t_max, hit_record& rec) const 
{
    ...
    // Rotating Ray in opposite direction
