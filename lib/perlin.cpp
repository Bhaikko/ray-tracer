...
double perlin::noise(const point3& p) const
{
    ...
    // Linearly Interpolating for smoothing between blocks
    vec3 c[2][2][2];
...
{
    // Adding Hermitian Smoothing to round off the interpolation
    // to avoid perceptual artifact of linear interpolation of color
    double uu = u * u * (3 - 2 * u);
    double vv = v * v * (3 - 2 * v);
    double ww = w * w * (3 - 2 * w);
    double accum = 0.0;
